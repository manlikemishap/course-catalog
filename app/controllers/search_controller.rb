class SearchController < ApplicationController
  ...
  def index
    ...
    # Map course to score
    #@results = {}
    @results = {}#Array.new(1100, 0) # number of courses
    ...
    dists.each do |dist|
      if dist == :q
        [229, 317, 319, 322, 328, 329, 334, 335, 346, 351, 360, 361, 362, 363, 364, 467, 468, 469, 470, 471, 472, 473, 474, 475, 476, 477, 478, 479, 482, 509, 510, 513, 517, 519, 520, 521, 522, 527, 529, 531, 532, 543, 556, 560, 630, 634, 656, 768, 769, 770, 771, 772, 773, 774, 775, 776, 777, 778, 779, 780, 781, 782, 783, 784, 785, 786, 787, 788, 791, 792, 793, 794, 795, 801, 802, 803, 804, 805, 806, 809, 810, 869, 880, 891, 892, 893, 894, 895, 896, 897, 898, 899, 900, 901, 902, 907, 913, 922, 960].each do |id|
          @results[id] = @results[id].nil? ? 2 : @results[id] * 2
        end
      elsif dist == :w
        [1, 3, 5, 15, 16, 20, 22, 35, 38, 41, 42, 51, 56, 61, 62, 63, 74, 75, 79, 81, 82, 96, 98, 99, 100, 101, 106, 108, 111, 123, 127, 130, 135, 136, 139, 150, 164, 165, 169, 194, 195, 198, 259, 260, 261, 274, 275, 302, 318, 320, 321, 330, 340, 352, 371, 390, 392, 393, 394, 417, 418, 419, 426, 428, 429, 430, 431, 432, 433, 434, 435, 437, 439, 441, 445, 516, 518, 526, 565, 566, 567, 568, 569, 570, 571, 572, 573, 574, 575, 577, 580, 582, 583, 584, 585, 586, 587, 588, 589, 590, 591, 602, 608, 611, 635, 641, 657, 672, 673, 674, 680, 681, 682, 683, 684, 703, 705, 715, 716, 724, 745, 747, 755, 756, 757, 795, 832, 836, 837, 863, 864, 865, 866, 867, 870, 872, 874, 875, 876, 877, 878, 879, 881, 882, 884, 885, 919, 929, 935, 936, 939, 941, 942, 950, 969, 970, 974, 1033, 1045, 1059, 1061].each do |id|
          @results[id] = @results[id].nil? ? 2 : @results[id] * 2
        end
      elsif dist == :d
        [2, 4, 9, 12, 15, 18, 21, 22, 23, 25, 27, 30, 31, 33, 38, 42, 43, 46, 47, 48, 49, 51, 52, 55, 60, 64, 65, 66, 69, 77, 78, 79, 83, 89, 90, 95, 97, 99, 100, 103, 105, 108, 111, 112, 113, 114, 121, 124, 128, 130, 131, 139, 151, 163, 180, 183, 219, 258, 271, 272, 275, 280, 281, 282, 283, 284, 285, 286, 287, 295, 296, 298, 299, 300, 303, 304, 305, 306, 392, 418, 420, 433, 440, 445, 458, 459, 502, 542, 577, 604, 614, 673, 683, 691, 700, 710, 713, 715, 716, 730, 736, 819, 823, 832, 837, 881, 974, 1029, 1033, 1059, 1060, 1061].each do |id|
          @results[id] = @results[id].nil? ? 2 : @results[id] * 2
        end
      end
    end

    # sort results. throw out the score, we dont need to display it anymore. then we can just work with the array
    # again.

    #@results = @results.map { |id| Course.find(id) }
    ...
    if false && divs.size > 1 || dists.size > 1 || params[:search]
